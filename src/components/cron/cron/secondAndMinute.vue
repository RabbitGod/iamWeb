...
<template lang="html">
  <div :val="value_">
    <RadioGroup  v-model="type">
      <div>
        <Radio label="1" size="small" border>每{{lable}}</Radio>
      </div>
      <div>
        <Radio label="2" size="small" border>周期</Radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <InputNumber @on-change="type = '2'" v-model="cycle.start" :min="1" :max="59" size="small" style="width: 100px;"></InputNumber>
        <span style="margin-left: 5px; margin-right: 5px;">至</span>
        <InputNumber @on-change="type = '2'" v-model="cycle.end" :min="2" :max="59" size="small" style="width: 100px;"></InputNumber>
        {{lable}}
      </div>
      <div>
        <Radio label="3" size="small" border>循环</Radio>
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <InputNumber @on-change="type = '3'" v-model="loop.start" :min="0" :max="59" size="small" style="width: 100px;"></InputNumber>
        <span style="margin-left: 5px; margin-right: 5px;">{{lable}}开始，每</span>
        <InputNumber @on-change="type = '3'" v-model="loop.end" :min="1" :max="59" size="small" style="width: 100px;"></InputNumber>
        {{lable}}执行一次
      </div>
      <div>
        <Radio label="4" size="small" border>指定</Radio>
        <CheckboxGroup v-model="appoint" @on-change="type = '4'">
          <div  v-for="i in 6" :key="i" style="margin-right: 10px; line-height: 25px;">
            <Checkbox  v-for="j in 10" :key="j" :label="(i - 1) + '' + (j - 1)"></Checkbox>
          </div>
        </CheckboxGroup>
      </div>
    </RadioGroup>
