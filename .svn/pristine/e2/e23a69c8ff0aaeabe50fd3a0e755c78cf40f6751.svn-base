<template lang="html">
  <div :val="value_">
    ...
    <div>
      <Radio label="1" size="small" border>每日</Radio>
    </div>
    <div>
    ...
    <div>
      ...
      <InputNumber @on-change="type = '2'" v-model="cycle.start" :min="1" :max="31" size="small" style="width: 100px;"></InputNumber>
      <span style="margin-left: 5px; margin-right: 5px;">至</span>
      <InputNumber @on-change="type = '2'" v-model="cycle.end" :min="2" :max="31" size="small" style="width: 100px;"></InputNumber>
      日
    </div>
    <div>
      <Radio label="3" size="small" border>循环</Radio>
      <span style="margin-left: 10px; margin-right: 5px;">从</span>
      <InputNumber @on-change="type = '3'" v-model="loop.start" :min="1" :max="31" size="small" style="width: 100px;"></InputNumber>
      <span style="margin-left: 5px; margin-right: 5px;">日开始，每</span>
      <InputNumber @on-change="type = '3'" v-model="loop.end" :min="1" :max="31" size="small" style="width: 100px;"></InputNumber>
      日执行一次
    </div>
    <div>
      <Radio label="8" size="small" border>工作日</Radio>
      <span style="margin-left: 10px; margin-right: 5px;"></span>
      <InputNumber @on-change="type = '8'" v-model="work" :min="1" :max="7" size="small" style="width: 100px;"></InputNumber>
      号，最近的工作日
    </div>
    <div>
      <Radio label="6" size="small" border>最后一天</Radio>
    </div>
    <div>
      <Radio label="4" size="small" border>指定</Radio>
      <CheckboxGroup  v-model="appoint" @on-change="type = '4'">
        <div  v-for="i in 4" :key="i" style="margin-left: 10px;  line-height: 25px;">
          <Checkbox   v-for="j in 10" v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)" :key="j" :label="(i - 1) + '' + (j - 1)"></Checkbox>
        </div>
      </CheckboxGroup >
    </div>
    </RadioGroup>
  ...
  methods: {
    updateVal () {
      ...
      if (this.value === '?') {
        ...
        this.type = '8'
        this.work = parseInt(this.value.replace('W', '')) 
      } else { // *
        this.type = '4'
